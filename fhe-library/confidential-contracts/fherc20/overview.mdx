...
Abstract contract providing claim functionality for managing unwrap operations with decrypt-with-proof verification.
</Card>
</CardGroup>
...
const [encryptedAmount] = await cofheClient
  ...
  .execute();
await token.confidentialTransfer(recipient, encryptedAmount);

// Check indicator (not real balance!)
const indicator = await token.balanceOf(user); // Returns indicator value

// Check real encrypted balance (only accessible to user)
const encBalance = await token.confidentialBalanceOf(user);
```
