...
contract SimpleCounter {
    ...
    function get_counter_value() external view returns(uint128) {
        (uint128 value, bool decrypted) = FHE.getDecryptResultSafe(lastDecryptedCounter);
        if (!decrypted)
...
This value is an encrypted value that we created using Cofhejs (read more about it [here](/cofhejs/guides/encryption)).
...
In the [next section](/cofhejs/examples/end-to-end) we will see how to use Cofhejs to privately decrypt this encrypted contract variable.

## Exercise

<Tip>
Try to modify the contract to allow the owner to read the counter's value without sending a transaction every time. You will need it in order to make the Cofhejs example work.
</Tip>

## Next Steps

- Learn how to use [Cofhejs](/cofhejs/examples/end-to-end) to interact with this contract
